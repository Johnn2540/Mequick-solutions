...
//
// Usage: node prisma/migrations/20260719180000_category_multi_image/apply.js
...
const MIGRATION_NAME = '20260719180000_category_multi_image';
...
async function main() {
  ...
  try {
    ...
    const before = await client.query('SELECT COUNT(*)::int AS count FROM "categories" WHERE "image" IS NOT NULL');
    console.log(`Categories with an image before migration: ${before.rows[0].count}`);
    ...
    try {
      ...
      const after = await client.query('SELECT COUNT(*)::int AS count FROM "category_images" WHERE "isPrimary" = true');
      console.log(`category_images rows created: ${after.rows[0].count}`);
      if (after.rows[0].count !== before.rows[0].count) {
        throw new Error(
          `Row count mismatch: ${before.rows[0].count} categories had images, but ${after.rows[0].count} category_images rows were created. Rolling back.`
        );
      }
